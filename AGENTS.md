...
The full submodule list with descriptions lives at https://v-sekai-fire.github.io/manuals/ and in [manuals/index.md](manuals/index.md).
...
Primary definition: `taskweft/priv/plans/domains/work_queue.jsonld`
...
To regenerate the plan: `cd taskweft && mix run -e '
...
### predictive-bvh — primary proof source
...
Location: `predictive-bvh/` (submodule in this repo)

## Critical cross-submodule relationships

### taskweft → multiplayer-fabric-artifacts-mmog / zone-console

`taskweft` is a library dependency of both the artifact CLI and the zone console. A breaking API change in taskweft requires updates in both consumers before either is released.
...
Chosen: `skills` (0.35) — highest probability, domain-neutral,
...
1. Adds remotes (`v-sekai-fire/godot` + `opentelemetry-godot/feat/open-telemetry-base`).
